...
public class UserManagerXX {
	
	/** Constractor
     * @param void
     */
	public UserManagerXX() {
	}
	...
	public int[] addUser(User user) {
		String sql = "INSERT INTO librisDB.`user` (user_ID,first_name, last_name, password, email, phone, `type`, enabled)" +
				"VALUES ("+user.getID()+",'" + user.getFirstName() + "', '"+ user.getLastName()+"', '"+user.getPassword() +"', '" + user.getEmailAddress()+ "', '6047253344', '', DEFAULT)";
		RequestPacket request = new RequestPacket();
