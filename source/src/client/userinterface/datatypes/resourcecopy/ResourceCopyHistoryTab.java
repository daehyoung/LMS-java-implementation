...
/**
 * Creates a tab to view a particular resouce copy's history.
 * 
 * @author Daniel Huettner
 * @version 5
 */
public class ResourceCopyHistoryTab extends JPanel {
	 ...
	 * Create the panel.
	 */
	public ResourceCopyHistoryTab(ResourceCopy resourceCopy) {
	...
	private boolean getData() {
		...
		if (loanArray == null) {
			Loan searchKey = new Loan(-1, resourceCopy.getID(), -1, null, null, null, -1, true);
			Loan[] loanArrayPaid = LoanManager.getLoan(searchKey);
			if (loanArrayPaid == null)
				return false;
			searchKey = new Loan(-1, resourceCopy.getID(), -1, null, null, null, -1, false);
