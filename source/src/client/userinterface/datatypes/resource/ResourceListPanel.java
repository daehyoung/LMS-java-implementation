...
public class ResourceListPanel extends JPanel {
	...
	private void createCompanyInput() {
		...
		companyInputField.setColumns(7);
	}
	...
	private void createCreatorInput() {
		...
		creatorInputField.setColumns(7);
	}
	...
	private boolean getData() {
		...
		if (SessionManager.getReference().getUser() instanceof StaffMember) {
			ResourceType searchKey = new ResourceType(-1, null, null, null, null, null, -1, -1, -1, -1, -1, true);
			ResourceType[] resourceTypesEnabled = ResourceTypeManager.getResourceType(searchKey);
			if (resourceTypesEnabled == null)
				return false;
			searchKey = new ResourceType(-1, null, null, null, null, null, -1, -1, -1, -1, -1, false);
			ResourceType[] resourceTypesDisabled = ResourceTypeManager.getResourceType(searchKey);
			if (resourceTypesDisabled == null)
				return false;
			resourceTypes = new ResourceType[resourceTypesEnabled.length + resourceTypesDisabled.length];
			int i;
			for (i = 0; i < resourceTypesEnabled.length; i++)
				resourceTypes[i] = resourceTypesEnabled[i];
			for (; i < resourceTypes.length; i++)
				resourceTypes[i] = resourceTypesDisabled[i - resourceTypesEnabled.length];
		}
		else {
