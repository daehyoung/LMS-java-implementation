...
public class LoanManager{
	...
	public static Loan addLoan(Loan newEntry){
		...
		*/
		...
		ResourceCopy resourceCopy = ResourceCopyManager.getResourceCopy(newEntry.getResourceCopy());
		if(!checkAdd(newEntry,resourceCopy))
			return null;
	...
	public static boolean checkInLoan(int idNumber){
		Loan loan = getLoan(idNumber);
		if(loan == null)
			return false;
		if(!loan.getFinePaid() && loan.getFineAmount() > 0)
			return removeLoan(idNumber);
		
		String sqlUpdate = "UPDATE loan SET check_in_date = UNIX_TIMESTAMP(NOW())*1000 WHERE " +
				"loan_ID = " + idNumber;
		...
		if(success){
			...
			if(resourceCopy != null)
				ReserveManager.setNextAvailableInQueue(resourceCopy.getResource());
		}
		return success;
	}
	...
	public static Loan renewLoan(Loan loan) {
		...
		try {
			int daysToLoan = getLoanDays(loan.getResourceCopy(), loan.getUserID());
			if(daysToLoan <= 0){
				ErrorManager.handleError(new ControlException("That type of resource cannot be renewed."));
				return loan;
			}
			String sqlUpdate = "UPDATE loan " +
					"SET " +
					"loan.due_date = (UNIX_TIMESTAMP(NOW()) + " + daysToLoan + "*24*60*60)*1000 " +
	 ...
	 * 
	 * @param newEntry The loan to add
	 * @param reserve 
	 * @return
	 */
	...
	private static boolean checkAdd(Loan entity, ResourceCopy resourceCopy){
		if(resourceCopy == null){
			ErrorManager.handleError(new ControlException("The given resource copy does not exist"));
			return false;
		}
		if(resourceCopy.getOwnerID() > 0){
			ErrorManager.handleError(new ControlException("The given resource copy does not belong to the library"));
			return false;
		}
			
		Reference[] reference = ReferenceManager.getReference(new Reference(-1,entity.getResourceCopy(),null,null,-1));
		if(reference != null && reference.length > 0 ){
			ErrorManager.handleError(new ControlException("This resource is on reference."));
			return false;
		}
			
		Loan[] loans = getLoan(new Loan(-1,-1,entity.getUserID(),null,null,null,-1,false));
		if(loans != null){
			for(Loan loan: loans){
				if(loan.getFineAmount() > 0){
					if(!ErrorManager.getConfirmation("This user still owes fines. Proceed anyways?"))
						return false;
				}
			}
		}
	...
	private static boolean checkDelete(int idNumber){
		Loan loan = getLoan(idNumber);
		if(loan == null){
			ErrorManager.handleError(new ControlException("Cannot find the given loan."));
			return false;			
		}
		if(!loan.getFinePaid() && loan.getFineAmount() > 0){
			ErrorManager.handleError(new ControlException("This loan cannot be deleted. The find must be paid first."));
			return false;			
		}
		return true;
	}
	...
	private static String buildInsertStatement(Loan entity) throws ControlException{
		...
		if(entity.getDueDate() != null){
			...
			int daysToLoan = getLoanDays(entity.getResourceCopy(),entity.getUserID());
			if(daysToLoan <= 0){
				throw new ControlException("That type of resource cannot be loaned out.");
			}
			attributes += "due_date, ";
			values +=  "(UNIX_TIMESTAMP(NOW()) + " + daysToLoan + "*24*60*60)*1000 " + ", ";
		}
