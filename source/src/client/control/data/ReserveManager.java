...
public class ReserveManager{
	private final static int DAYS_TO_EXPIRE = 7;		//number of days a reservation is valid for
	...
	public static boolean removeReserve(int idNumber){
		...
		*/
		...
		if(success){
			if(reserve != null && reserve.getAvailableDate() != null)
				ReserveManager.setNextAvailableInQueue(reserve.getResource());
		}
		return success;
	}
	
	protected static boolean setNextAvailableInQueue(int resourceID){
		String sqlUpdate = "UPDATE reservation, " +
				"(SELECT reservation_ID, MIN(reserved_date) " +
					...
					"resource_resource_ID = " + resourceID + " ) AS subtable " +
				"SET available_date = UNIX_TIMESTAMP(NOW())*1000," +
				"expire_date = (UNIX_TIMESTAMP(NOW())*1000 + 24*60*60*1000*" + DAYS_TO_EXPIRE + ") " + 
				"WHERE " +
		...
		try{
			...
			for(int i=0; i < nRows && rowSet.next();i++){
				result[i] = new Reserve(
						...
						rowSet.getLong("available_date") != 0 ? new Date(rowSet.getLong("available_date")) : null,
