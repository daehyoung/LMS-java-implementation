...
public class ResourceManager{
	...
	private static boolean check(Resource newEntry) {
		Resource[] sameTitle = getResource(new Resource(0,0,newEntry.getTitle(),null,null,null,null,true));
		if(sameTitle != null && sameTitle.length > 0) {
			if(!ErrorManager.getConfirmation("This title already exists. Do you want to continue?"))
				return false;
		}
//Peter: I noticed serial is being used as "Rating" for a DvD so not sure if we should check for duplicates
/*		Resource[] sameSerial = getResource(new Resource(0,newEntry.getResourceType(),null,null,null,newEntry.getSerialNumber(),null,true));
		
		if(sameSerial != null && sameSerial.length > 0) {
			if(!ErrorManager.getConfirmation("This serial number is in use by a resource of the same type. Do you want to continue?"))
		      return false;
		}
*/		
		return true;
	...
	public static boolean setResource(Resource newEntry){
		...
		if(!newEntry.getEnabled()) {
			Reserve[] r = ReserveManager.getReserve(new Reserve(0, 0, newEntry.getID(), null,null,null));
			if(r.length > 0) {
				if (JOptionPane.showConfirmDialog(new JFrame(),
				        "Reservation exits for this resource. Do you want to continue to disable?", "Title",
				        JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
						for(int i=0; i<r.length; i++) {
							ReserveManager.removeReserve(r[i].getID());
						}
	...
	public static void main(String args[]) throws ConnectException{
		Resource r = new Resource(83, 81, "SardorII", "Francis Ford Coppola", "Langara Pictures", "OO26", new Date(0), false);
...
/*		addResource(r);
		...
		Resource r1 = new Resource(82, 81, "SardorII", "Francis Ford Coppola", "Langara Pictures", "OO26", new Date(0), false);
		setResource(r1);
		System.out.println(buildUpdateStatement(r1));
