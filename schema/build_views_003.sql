...
CREATE VIEW `librisDB`.`resourceSearch` AS
	SELECT
		resource.resource_ID,resource.title,resource.creator,resource.year,resource.description,resource.company,resource.resourceType_type_ID,COUNT(resourceCopy.copy_ID)'available'
	FROM
...
-- -----------------------------------------------------
-- Creates a view of a user
-- -----------------------------------------------------
DROP VIEW IF EXISTS `librisDB`.`userView`;
CREATE VIEW `librisDB`.`userView` AS
	SELECT
		user.user_ID, user.first_name, user.last_name, user.email, user.phone, user.type, user.enabled
	FROM
		user;